...
echo -e "\n# Non-Astra-DB additional setting (local backend URL):\nJAVA_GRAPHQL_ENDPOINT=\"$URL/graphql\"" > /workspace/workshop-intro-to-graphql/graphql-client-examples/.local-backend.env
# also writing here to have a local-only-working React app from the start
echo -e "\n# Non-Astra-DB additional setting (local backend URL):\nJAVA_GRAPHQL_ENDPOINT=\"$URL/graphql\"" > /workspace/workshop-intro-to-graphql/graphql-client-examples/.env
